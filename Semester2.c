...
void findRecord(int id)
{
	...
	while (current != NULL)
	{
		if (current->ID == id)
		{
			printf("%d - %s %s Maths<%c> English<%c> French<%c> Philosophy<%c>", current->ID, current->firstname, current->surname, current->maths, current->english, current->french, current->philosophy);
			return;
		}
